...
def calc_frechet(polygon, reference):
    ...
    if isinstance(polygon, (Polygon)) and isinstance(reference, (Polygon)):
        return polygon.frechet_distance(reference)
    return float('inf')
...
def grid_search(pand, reference_column, aligned_column, alpha, angle_step=0.5, scale_step=0.05, scale_range=(0.9, 1.1)):
    ...
    for angle in angles:
        transformed_geometries = pand[aligned_column].apply(
            lambda g: rotate(g, angle, origin='centroid'))
        ...
        if score > best_score:
            ...
            best_angle = angle
            best_gof = score_gof
            best_haus = score_haus

    pand['optimized_geometry'] = best_geometries
    pand['score_gof'] = best_gof
    pand['score_haus'] = score_haus
    pand['score'] = best_score
    pand['angle'] = best_angle
    haus_values.append(best_haus)
    gof_values.append(best_gof)
...
panden2.to_file(os.path.join("werkmap", f"hausdorff_norot_{alpha}.shp"), driver="ESRI Shapefile")
