...
def rigid_transform_polygon(aligned_geom, matched_aligned, matched_ref, reference_geom, max_scale=1.2,
                            max_translation=5):
    """Compute a rigid transformation (no skew) and apply it to the polygon."""
    if len(matched_aligned) < 3:
    ...
    # Extract scale from matrix (ensuring uniform scaling)
    scale = np.linalg.norm(transform.params[0, :2])
    if scale > max_scale:
        print("Transformation rejected: excessive scaling.")
        return aligned_geom  # Reject extreme scaling
    ...
    # Ensure transformed polygon stays within a buffered reference boundary
    buffer_distance = max_translation
    reference_buffer = reference_geom.buffer(buffer_distance)

    if not transformed_polygon.within(reference_buffer):
        print("Transformation rejected: out of bounds.")
        return aligned_geom  # Reject transformation if out of bounds

    return transformed_polygon


def refine_alignment(reference_geom, aligned_geom, distance_threshold=1.5, angle_threshold=1, max_scale=1.2,
                     max_translation=4):
...
panden2.to_file(os.path.join("werkmap", f"snap_test_othermet_rigid15.shp"), driver="ESRI Shapefile")
