...
scale_versions = ['text', 'area']
rotate_versions = ['azimuth', 'arrow']
translation_versions = ['bbox', 'centroid']

# List of rotation angles (if needed in tests)
rotation_angles = {'HVS00N1878': 171.3, "HVS00N1882": 180, "HVS00N2359": -43.5,
                   "HVS00N2643": 78.9, "HVS00N2848": 6.8, "HVS00N3211": 0.0,
                   "HVS00N3723": 121.6, "HVS00N4216": 120, "HVS00N555": 22.2,
                   "HVS00N9252": 7}

# Create all combinations of the options
combinations = list(itertools.product(scale_versions, rotate_versions, translation_versions))

# Initialize a list to store the results for each combination
results = []

# Iterate through each combination
for scale_version, rotate_version, translation_version in combinations:
    print(f"Testing combination: Scale={scale_version}, Rotate={rotate_version}, Translation={translation_version}")
    ...
    for perceel in perceel_list:
        ...
        pand = gp.GeoDataFrame(pand, geometry='bgt_outline')
        ...
        if translation_version == 'centroid':
            if len(pand.bgt_outline.centroid) > 1:
                bgt_centroid = pand.bgt_outline.centroid.iloc[0]
            else:
                bgt_centroid = pand.bgt_outline.centroid
        ...
        avg_gof = pand['gof'].sum() / len(pand)
        results.append((scale_version, rotate_version, translation_version, avg_gof))

    # Collect the result for this combination


    # Optionally, save or process results
    print(f"Average GOF for combination {scale_version}, {rotate_version}, {translation_version}: {avg_gof}")
    ...
    # panden2.to_file("gof2_" + rotate_version + "_" + translation_version + "_" + scale_version + ".shp",
    #                 driver="ESRI Shapefile")

# Optionally plot the results
import matplotlib.pyplot as plt

results_df = pd.DataFrame(results, columns=['Scale', 'Rotate', 'Translate', 'Avg_GO'])
results_df.to_csv("combined_results.csv", index=False)

# Optionally plot the results
import matplotlib.pyplot as plt

# Create a color-coded plot based on results
fig, ax = plt.subplots()
for (scale_version, rotate_version, translation_version), group in results_df.groupby(['Scale', 'Rotate', 'Translate']):
    ax.barh(f"{scale_version}-{rotate_version}-{translation_version}", group['Avg_GO'].mean())

ax.set_xlabel('Average GOF')
ax.set_title('Average GOF for different combinations')
plt.tight_layout()
plt.show()